...
def lambda_handler(event, context):
        try:
            try:
                lf_admin_role_arn = os.environ['LF_ADMIN_ROLE_ARN']
                logger.info(f"Using Lake Formation Admin Role: {lf_admin_role_arn}")
            except KeyError:
                logger.error("Missing required environment variable: LF_ADMIN_ROLE_ARN")
                return {
                    'statusCode': 500,
                    'body': json.dumps({
                        'error': 'Configuration Error',
                        'message': 'Missing required environment variable: LF_ADMIN_ROLE_ARN'
                    })
                }
            ...
            # Initialize LF Tag Manager with the role ARN
            lf_manager = LFTagManager(lf_admin_role_arn)
            ...
            # Parse and format the response for logging
            formatted_response = {
                'statusCode': response['statusCode'],
                'body': json.loads(response['body'])
            }

            # Log the successful completion with formatted result
            logger.info(f"Processing completed successfully. Result:\n{json.dumps(formatted_response, indent=2)}")

            # Return successful response with changes summary
